...
// The original code is here: https://github.com/manuelnas/CesiumHeatmap/blob/master/HeatmapImageryByContour.js
...
function createHeatmapImageryByContour(Cesium, options) {

    var HeatmapImageryByContour = function (options) {
        ...
        var bounds;
        if (Cesium.defined(options.bounds)) {
            bounds = options.bounds;
        }
        var clip;
        var mask;
        if (Cesium.defined(options.clip)) {
            clip = options.clip;
            //  console.log(clip)
        }
        if (Cesium.defined(options.mask)) {
            mask = options.mask;
            // console.log(mask)
        }
        ...
        var chInstance = options.chInstance || h338;


        this._wmp = new Cesium.WebMercatorProjection();
        this._mbounds = this.wgs84ToMercatorBB(bounds);
        this._options.gradient = Cesium.defaultValue(this._options.gradient, {
            0.25: "rgb(0,0,255)",
            0.55: "rgb(0,255,0)",
            0.85: "yellow",
            1.0: "rgb(255,0,0)"
        ...
        this._ready = this.setWGS84DataContour(min, max, arrPoint, clip, mask);
        this.readyPromise.resolve(true);
    };

    Cesium.defineProperties(HeatmapImageryByContour.prototype, {
        /**
         * Gets the URL of the single, top-level imagery tile.
         * @memberof HeatmapImageryByContour.prototype
         * @type {String}
         * @readonly
         */
        url: {
         ...
         * not be called before {@link HeatmapImageryByContour#ready} returns true.
         * @memberof HeatmapImageryByContour.prototype
         * @type {Number}
         * @readonly
         */
         ...
         * the source of the imagery.  This function should not be called before {@link HeatmapImageryByContour#ready} returns true.
         * @memberof HeatmapImageryByContour.prototype
         * @type {Credit}
         * @readonly
         */
    ...
    HeatmapImageryByContour.prototype._setWidthAndHeight = function (mbb) {
    ...
    HeatmapImageryByContour.prototype._getContainer = function (width, height, id) {
     ...
     */
    HeatmapImageryByContour.prototype.wgs84ToMercator = function (point) {
        return this._wmp.project(Cesium.Cartographic.fromDegrees(point[this._options.xField], point[this._options.yField]));
    };

    HeatmapImageryByContour.prototype.wgs84ToMercator4Contour = function (point) {
        return this._wmp.project(Cesium.Cartographic.fromDegrees(point.lng, point.lat));
    };
     ...
     */
    HeatmapImageryByContour.prototype.wgs84ToMercatorBB = function (bounds) {
     ...
     */
    HeatmapImageryByContour.prototype.mercatorToWgs84 = function (p) {
     ...
     */
    HeatmapImageryByContour.prototype.mercatorToWgs84BB = function (bounds) {
     ...
     */
    HeatmapImageryByContour.prototype.deg2rad = function (degrees) {
     ...
     */
    HeatmapImageryByContour.prototype.rad2deg = function (radians) {
     ...
     */
    HeatmapImageryByContour.prototype.wgs84PointToHeatmapPoint = function (point) {
        return this.mercatorPointToHeatmapPoint(this.wgs84ToMercator(point));
    };
    HeatmapImageryByContour.prototype.wgs84ContourToHeatmapContour = function (point) {
        return this.mercatorPointToHeatmapPoint4Contour(this.wgs84ToMercator4Contour(point));
    };
     ...
     * @returns {Object} The corresponding heatmap location.
     */
    HeatmapImageryByContour.prototype.mercatorPointToHeatmapPoint = function (point) {
        var pn = {};
    ...
    HeatmapImageryByContour.prototype.mercatorPointToHeatmapPoint4Contour = function (point) {
        var pn = {};
     ...
     */
    HeatmapImageryByContour.prototype.setData = function (min, max, data, clip, mask) {
     ...
     * @returns {Boolean} Wheter or not the data was successfully added or failed.
     */
    HeatmapImageryByContour.prototype.setWGS84Data = function (min, max, data) {
    ...
    HeatmapImageryByContour.prototype.setWGS84DataContour = function (min, max, data, clip, mask) {
        if (data && data.length > 0 && min !== null && min !== false && max !== null && max !== false) {
            let result = [];
            var masks = [];
            var clips = [];
            // console.log(data, clip, mask)
            data.forEach((contour) => {
                ...
                temp.lines = contour.lines.map((line) => {
                    return line.map((point) => this.wgs84ContourToHeatmapContour(point));
                });
                result.push(temp)
                // result[contour.value] = contour.lines.map((line) => {
                //     return line.map((point) => this.wgs84ContourToHeatmapContour(point));
                // });
            });
            // console.log(result)
            ...
            if (clip && clip.length > 0) {
                clips = clip.map(point => {
                    return this.wgs84ContourToHeatmapContour(point)
                });
              //  debugger
                //  clips = clip.map((point) => this.wgs84ContourToHeatmapContour(point));
            }
            // console.log(result, clips, masks)
            return this.setData(min, max, result, clips, masks);
        }
        return false;
    };
     ...
     */
    HeatmapImageryByContour.prototype.getTileCredits = function (x, y, level) {
        return undefined;
    };
     ...
     */
    HeatmapImageryByContour.prototype.requestImage = function (x, y, level) {
        if (!this._ready) {
     ...
     */
    HeatmapImageryByContour.prototype.pickFeatures = function () {
        return undefined;
    };

    return new HeatmapImageryByContour(options);
}